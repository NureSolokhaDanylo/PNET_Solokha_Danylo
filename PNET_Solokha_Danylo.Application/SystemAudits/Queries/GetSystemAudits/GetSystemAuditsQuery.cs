...
) : IRequestHandler<GetSystemAuditsQuery, SystemAuditQueryResult>
{
    ...
    {
        logger.LogInformation("Handling GetSystemAuditsQuery: SearchTerm={SearchTerm}, Severity={Severity}, ActionType={ActionType}, TableName={TableName}, StartDate={StartDate}, EndDate={EndDate}, Skip={Skip}, Take={Take}",
        ...
        try
        {
            ...
            logger.LogInformation("Successfully fetched {Count} (out of {Total}) SystemAudits.", items.Count, totalCount);
            return new SystemAuditQueryResult(items, totalCount);
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Error occurred while handling GetSystemAuditsQuery.");
            throw;
        }
    }
}
